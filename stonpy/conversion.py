...
def map_to_subgraph(
    ...
    if sbgn_map.get_extension() is not None:
        ...
        if make_sbml_annotations and _extension_has_sbml_annotation(
            sbgn_map.get_extension()
        ):
            (
                annotation_nodes,
                annotation_other_nodes,
                annotation_relationships,
            ) = _extension_as_sbml_annotation_to_subgraph(
                sbgn_map.get_extension()
            )
            ...
            for annotation_node in annotation_nodes:
                relationships.add(
                    Relationship(
                        map_node,
                        STONEnum["HAS_ANNOTATION"].value,
                        annotation_node,
                    )
                )
    if sbgn_map.get_notes() is not None:
        map_node[STONEnum["NOTES"].value] = str(
            utils.decode_notes_and_extension(sbgn_map.get_notes())
        )
    ...
    for glyph in sbgn_map.get_glyph():
        if glyph.get_class().name == "COMPARTMENT":
            ...
            relationships.add(
                Relationship(map_node, STONEnum["HAS_GLYPH"].value, glyph_node)
            )
    ...
    for arc in sbgn_map.get_arc():
        if (
            arc.get_class().name == "ASSIGNMENT"
            or arc.get_class().name == "INTERACTION"
        ):
            ...
            relationships.add(
                Relationship(map_node, STONEnum["HAS_ARC"].value, arc_node)
            )
            dids[arc.get_id()] = arc_node
    ...
    for arc in sbgn_map.get_arc():
        if (
            arc.get_class().name != "ASSIGNMENT"
            and arc.get_class().name != "INTERACTION"
        ):
            arc_node, arc_other_nodes, arc_relationships = _arc_to_subgraph(
                arc,
                dids,
    ...
    for arcgroup in sbgn_map.get_arcgroup():
        (
            arcgroup_node,
            arcgroup_other_nodes,
            arcgroup_relationships,
        ) = _arcgroup_to_subgraph(
            arcgroup,
            dids,
            dpids,
            ontology,
    ...
    if len(rdf_string) > 0:
        ...
        for s, p, o in rdf_graph:
            if (
                ...
                and isinstance(s, rdflib.term.BNode)
                and not str(p).endswith("#type")
            ):
                resource_node = Node()
...
def _glyph_to_subgraph(
    ...
    if glyph.get_extension() is not None:
        ...
        if make_sbml_annotations and _extension_has_sbml_annotation(
            ...
            (
                ...
                annotation_other_nodes,
                annotation_relationships,
            ) = _extension_as_sbml_annotation_to_subgraph(glyph.get_extension())
            nodes |= annotation_nodes
            nodes |= annotation_other_nodes
            relationships |= annotation_relationships
            for annotation_node in annotation_nodes:
    ...
    if glyph.get_label() is not None:
        ...
        label_node, label_other_nodes, label_relationships = _label_to_subgraph(
            label
        )
    ...
    if glyph.get_compartmentRef():
        is_in_compartment = Relationship(
            node,
    ...
    for i, subglyph in enumerate(svs):
        ...
        (
            subglyph_node,
            subglyph_other_nodes,
            subglyph_relationships,
        ) = _glyph_to_subgraph(
            subglyph,
            dids,
            dpids,
    ...
    for subglyph in glyph.get_glyph() + svs:
        sub_ston_type = subglyph.get_class().name
        if (
            sub_ston_type == "UNIT_OF_INFORMATION"
            or sub_ston_type == "TERMINAL"
            or sub_ston_type == "OUTCOME"
            or sub_ston_type == "EXISTENCE"
            or sub_ston_type == "LOCATION"
        ):
            (
                subglyph_node,
                subglyph_other_nodes,
        ...
        dids[subglyph.get_id()] = subglyph_node
        relationships.add(
            Relationship(node, STONEnum[r_type].value, subglyph_node)
        )
    ...
    for port in glyph.get_port():
        ...
        nodes.add(port_node)
        relationships.add(
            Relationship(node, STONEnum["HAS_PORT"].value, port_node)
        )
...
def _arc_to_subgraph(
    arc, dids, dpids, ontology, make_shortcuts=True, make_sbml_annotations=True
):
    node = Node()
    nodes = set([])
    relationships = set([])
    ...
    if arc.get_extension() is not None:
        ...
        if make_sbml_annotations and _extension_has_sbml_annotation(
            ...
            (
                ...
                annotation_other_nodes,
                annotation_relationships,
            ) = _extension_as_sbml_annotation_to_subgraph(arc.get_extension())
            nodes |= annotation_nodes
            nodes |= annotation_other_nodes
            relationships |= annotation_relationships
            for annotation_node in annotation_nodes:
    ...
    for i, nextt in enumerate(arc.get_next()):
        ...
        relationships.add(
            Relationship(node, STONEnum["HAS_NEXT"].value, next_node)
        )
    ...
    for glyph in arc.get_glyph():
        ...
        if glyph.get_class().name == "CARDINALITY":
            ...
            relationships.add(
                Relationship(node, STONEnum["HAS_OUTCOME"].value, glyph_node)
            )
    ...
    if make_shortcuts:
        if "{}_{}".format(ston_type, "SHORTCUT") in [
            ...
            if cardinality is not None:
                ...
                props = {}
            if (
                ston_type == "CONSUMPTION"
                or ston_type == "LOGIC_ARC"
                or ston_type == "EQUIVALENCE_ARC"
            ):
...
def _arcgroup_to_subgraph(
    ...
    if (
        ston_type == "INTERACTION_ARCGROUP"
    ...
    if arcgroup.get_notes():
        node[STONEnum["NOTES"].value] = str(
            utils.decode_notes_and_extension(arcgroup.get_notes())
        )
    ...
    for glyph in arcgroup.get_glyph():
        ...
        relationships |= glyph_relationships
        relationships.add(
            Relationship(node, STONEnum["HAS_GLYPH"].value, glyph_node)
        )
    for arc in arcgroup.get_arc():
        ...
        relationships |= arc_relationships
        relationships.add(
            Relationship(node, STONEnum["HAS_ARC"].value, arc_node)
        )
    return node, nodes, relationships
...
def subgraph_to_map(subgraph):
    ...
    for node in subgraph.nodes:
        if node.has_label(STONEnum["MAP"].value):
            ...
            if node[STONEnum["NOTES"].value]:
                sbgn_map.set_notes(
                    libsbgnSubs.Notes(node[STONEnum["NOTES"].value])
                )
            if node[STONEnum["EXTENSION"].value]:
                sbgn_map.set_extension(
                    libsbgnSubs.Extension(node[STONEnum["EXTENSION"].value])
                )
    ...
    for relationship in subgraph.relationships:
        ...
        if rtype == STONEnum["HAS_BBOX"].value:
            ...
            rtype == STONEnum["HAS_GLYPH"].value
            or rtype == STONEnum["HAS_SUBUNIT"].value
            or rtype == STONEnum["HAS_STATE_VARIABLE"].value
            or rtype == STONEnum["HAS_EXISTENCE"].value
            or rtype == STONEnum["HAS_LOCATION"].value
            or rtype == STONEnum["HAS_UNIT_OF_INFORMATION"].value
            or rtype == STONEnum["HAS_TERMINAL"].value
            or rtype == STONEnum["HAS_OUTCOME"].value
            or rtype == STONEnum["HAS_CARDINALITY"].value
        ):
