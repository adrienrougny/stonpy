...
def complete_subgraph(subgraph, neograph):
    ...
    for relationship in subgraph.relationships:
        subgraph |= complete_subgraph_with_relationship(subgraph, relationship, completed, neograph)
    for node in subgraph.nodes:
        if node not in completed:
            ...
            elif node.has_label(STONEnum["ARC"].value): subgraph |= complete_subgraph_with_arc_node(subgraph, node, completed, neograph)
...
def complete_subgraph_with_relationship(subgraph, relationship, completed, neograph):
    ...
    if STONEnum(r_type).name.endswith(shortcut):
        ...
        queries = []
        queries.append('OPTIONAL MATCH {source}<-[:{has_source}]-(arc:{arc_label})-[:{has_target}]-{target} RETURN arc'.format(
            **{
                "source": source,
        ...
        queries.append('OPTIONAL MATCH {source}-[:{has_port}]->()<-[:{has_source}]-(arc:{arc_label})-[:{has_target}]->{target} RETURN arc'.format(
            **{
                "source": source,
                "has_source": STONEnum["HAS_SOURCE"].value,
                "arc_label": arc_label,
        ...
        queries.append('OPTIONAL MATCH {source}<-[:{has_source}]-(arc:{arc_label})-[:{has_target}]->()<-[:{has_port}]-{target} RETURN arc'.format(
            **{
                "source": source,
                "has_source": STONEnum["HAS_SOURCE"].value,
                "arc_label": arc_label,
...
def complete_subgraph_with_glyph_node(subgraph, node, completed, neograph):
    completed.add(node)
    #BBOX
    relationship = utils.match_one(subgraph, (node,), STONEnum["HAS_BBOX"].value)
    ...
    #MAP
    relationship = utils.match_one(subgraph, (None, node), STONEnum["HAS_GLYPH"].value)
    if relationship is None:
    ...
    for name in ["HAS_STATE_VARIABLE", "HAS_UNIT_OF_INFORMATION", "HAS_SUBUNIT", "HAS_OUTCOME"]:
        relationship = utils.match_one(subgraph, (None, node), STONEnum[name].value)
        if relationship is None:
...
def complete_subgraph_with_port_node(subgraph, port_node, completed, neograph):
    ...
    #OWNER
    relationship = utils.match_one(subgraph, (None, port_node), STONEnum["HAS_PORT"].value)
    if relationship is None:
...
def complete_subgraph_with_bbox_node(subgraph, bbox_node, completed, neograph):
    ...
    relationship = utils.match_one(subgraph, (None, bbox_node), STONEnum["HAS_BBOX"].value)
    if relationship is None:
...
def complete_subgraph_with_arc_node(subgraph, node, completed, neograph):
    ...
    for relationship in relationships:
        ...
        if subglyph_node not in completed:
            subgraph |= complete_subgraph_with_glyph_node(subgraph, subglyph_node, completed, neograph)
    #MAP
    relationship = utils.match_one(subgraph, (None, node), STONEnum["HAS_ARC"].value)
    if relationship is None:
    ...
    if relationship is not None:
        ...
        subgraph |= sbgnmap
    #CARDINALITY
    relationship = utils.match_one(subgraph, (node,), STONEnum["HAS_CARDINALITY"].value)
    if relationship is None:
    ...
    #SOURCE
    relationship = utils.match_one(subgraph, (node,), STONEnum["HAS_SOURCE"].value)
    if relationship is None:
    ...
    #TARGET
    relationship = utils.match_one(subgraph, (node,), STONEnum["HAS_TARGET"].value)
    if relationship is None:
    ...
    #START
    relationship = utils.match_one(subgraph, (node,), STONEnum["HAS_START"].value)
    if relationship is None:
    ...
    if relationship is not None:
        ...
        subgraph |= start 
    #END
    relationship = utils.match_one(subgraph, (node,), STONEnum["HAS_END"].value)
    if relationship is None:
...
def complete_subgraph_with_arcgroup_node(subgraph, node, completed, neograph):
    ...
    #MAP
    relationship = utils.match_one(subgraph, (None, node), STONEnum["HAS_ARCGROUP"].value)
    if relationship is None:
